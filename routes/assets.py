...
async def asset_production(
    ...
    elif kind == "static-assets":
        # Not a real addressables bundle kind -- "platform" and "version" here
        # are actually just real folder names (e.g. Resources/Textures), not a
        # platform+discardable-version pair, so the full nested path matters.
        if local_assets_enabled():
            local = static_asset(f"{platform}/{version}/{filepath}")
            if local is not None:
                return _octet(local)
