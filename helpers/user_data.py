...
def _conv(base, is_array, kind, v):
    ...
    if is_array:
        ...
        # MessagePack byte[] must be represented as Python bytes,
        # otherwise msgpack will encode it as an integer array.
        if base == "byte":
...
def _to_array(type_name: str, value: dict) -> list:
    if type_name in ("AuditionClear", "MissionPass") and "wireUserId" in value:
        value = dict(value)
        value["userId"] = value["wireUserId"]
    keys = KEYS[type_name]
    if not keys:
        return []
