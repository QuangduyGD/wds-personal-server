...
def static_asset(rel_path: str) -> Optional[Tuple[bytes, str]]:
    """A local static-content file (UI banners, comics, etc.) under
    _data/assets/static-assets/, or None if not downloaded.

    Unlike bundle(), static-assets URLs don't have a discardable "version"
    segment -- the full path after /production/static-assets/ (e.g.
    Resources/Textures/Banners/Gacha/3028.astc.gz) is the real on-disk path,
    case preserved as-is (no .lower() -- Windows is case-insensitive but this
    should also work correctly if ever run on a case-sensitive filesystem)."""
